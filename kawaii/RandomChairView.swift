...
struct RandomChairView: View {
            ...
            ZStack {
                ...
                */
                ...
                VStack {
                    ...
                    if !hasBeenTapped {
                        Text("Touch the Touch Screen to see a Cuck")
    ...
    private func handleScreenTap() {
        ...
        // Add chair photo directly on any tap
        print("Screen tapped! Adding chair photo...")
        addChairPhoto()
    }
    ...
    private func addChairPhoto() {
        // Randomly choose between chair images
        let chairImageNames = ["chair", "chair2"]
        let randomChairName = chairImageNames.randomElement() ?? "chair"
        
        guard let chairImage = UIImage(named: randomChairName) else {
            print("Chair image '\(randomChairName)' not found in assets")
            return
        }
        ...
        // Create PhotoItem with chair image
        let photoItem = PhotoItem(image: chairImage, position: position)
        
        // Add to photo items
        photoItemsViewModel.photoItems.append(photoItem)
        
        // Play random sound from the 2 chair sounds
        let chairSounds: [SoundType] = [.chair1, .chair2]
        let randomSound = chairSounds.randomElement() ?? .chair1
        soundService.playSound(randomSound)
        
        print("Added chair photo at position: \(position)")
    }
